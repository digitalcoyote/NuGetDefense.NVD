...
public class Client : IDisposable
{
    ...
    public Client(string userAgent, string? apiKey = null)
    {
        _apiKey = apiKey;
        client = new();
    ...
    public async Task<CveResponse?> GetCvesAsync(CvesRequestOptions options)
    {
        ...
        if (options.CpeName != null)
        {
            queryStringParams.Add($"cpeName={HttpUtility.UrlEncode(options.CpeName)}");
        }
        
        if (options.CveId != null)
        {
            queryStringParams.Add($"cveId={HttpUtility.UrlEncode(options.CveId)}");
        }
        
        if (options.CweId != null)
        {
            queryStringParams.Add($"cweId={HttpUtility.UrlEncode(options.CweId)}");
        }
        ...
        {
            queryStringParams.Add($"cvssV2Severity={HttpUtility.UrlEncode(Helpers.Cvss2SeverityToString(options.CvssV2Severity))}");
        }
        ...
        {
            queryStringParams.Add($"cvssV3Severity={HttpUtility.UrlEncode(Helpers.Cvss3SeverityToString(options.CvssV3Severity))}");
        }
        ...
        {
            queryStringParams.Add($"versionEndType={HttpUtility.UrlEncode(Helpers.VersionEndTypeToString(options.VersionEndType))}");
        }
        ...
        {
            queryStringParams.Add($"versionStartType={HttpUtility.UrlEncode(Helpers.VersionStartTypeToString(options.VersionStartType))}");
        }
        
        if (options.HasCertAlerts)
        {
            queryStringParams.Add("hasCertAlerts");
        }
        
        if (options.HasCertNotes)
        {
            queryStringParams.Add("hasCertNotes");
        }
        
        if (options.HasKev)
        {
            queryStringParams.Add("hasKev");
        }
        
        if (options.HasOval)
        {
            queryStringParams.Add("hasOval");
        }
        
        if (options.IsVulnerable)
        {
            queryStringParams.Add("isVulnerable");
        }
        
        if (options.KeywordExactMatch)
        {
            queryStringParams.Add("keywordExactMatch");
        }
        
        if (options.NoRejected)
        {
            queryStringParams.Add("noRejected");
        }
        ...
        {
            queryStringParams.Add($"lastModStartDate={HttpUtility.UrlEncode(((DateTime)options.LastModStartDate).ToString("O"))}");
        }
        
        if (options.LastModEndDate != null)
        {
            queryStringParams.Add($"lastModEndDate={HttpUtility.UrlEncode(((DateTime)options.LastModEndDate).ToString("O"))}");
        }

        
        if (options.PubStartDate != null)
        {
            queryStringParams.Add($"pubStartDate={HttpUtility.UrlEncode(((DateTime)options.PubStartDate).ToString("O"))}");
        }
        ...
        {
            throw new Exception($"Failed to get CVE from NVD with error: {response.ReasonPhrase}");
        }

        return await JsonSerializer.DeserializeAsync<CveResponse>(await response.Content.ReadAsStreamAsync());
